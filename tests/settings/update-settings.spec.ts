...
test.describe('Update user settings', { tag: '@settings' }, () => {
  test('updates the profile and credentials', { tag: '@positive' }, async ({
    page, settingsPage, profilePage, isolatedUser,
  }) => {
    const settings = buildUserSettings();

    await test.step('open settings from the header', async () => {
      await page.goto('/');
      await settingsPage.header.settingsLink.click();
  ...
  test('header shows the new username right after saving', {
    tag: ['@positive', '@known-issue'],
    annotation: {
      type: 'issue',
      description: 'After "Update Settings" the header nav renders empty (no user or sign-in links) until the page is reloaded.',
    },
  ...
  test('rejects a username that is already taken', { tag: '@negative' }, async ({
    page, settingsPage, otherUser, isolatedUser,
  }) => {
    await settingsPage.open();
