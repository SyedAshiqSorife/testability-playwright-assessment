...
test.describe('Delete article', { tag: '@delete-article' }, () => {
  test(
    'deletes an article and removes it everywhere',
    { tag: '@positive' },
    async ({ page, homePage, articlePage, profilePage, api, articles, sessionUser }) => {
      ...
      await test.step('redirects to the home page feed', async () => {
      ...
      await test.step("article is gone from the author's profile", async () => {
        ...
        await expect(profilePage.articleTitle(article.title)).toHaveCount(0);
      });
