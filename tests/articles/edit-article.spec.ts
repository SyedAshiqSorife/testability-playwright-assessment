...
test.describe('Edit article', { tag: '@edit-article' }, () => {
  test(
    'updates an existing article and persists the changes',
    { tag: '@positive' },
    async ({ page, articlePage, editorPage, api, articles, sessionUser }) => {
      ...
      await test.step('changes are persisted and survive a reload', async () => {
        ...
        expect(saved.tagList.toSorted()).toEqual(expectedTags.toSorted());
  ...
  test(
    'clearing the title does not wipe the saved title',
    { tag: '@negative' },
    async ({ page, editorPage, articlePage, api, articles }) => {
      ...
      await expect(page).toHaveURL(`/article/${original.slug}`);
  ...
  test(
    "cannot edit another author's article",
    { tag: '@negative' },
    async ({ articlePage, api, otherUser, otherUserArticles }) => {
      ...
      await test.step('API refuses the update and the article is unchanged', async () => {
        ...
        expect(await response.json()).toMatchObject({
          message: expect.stringMatching(Messages.permissions.notAuthorized),
        });
